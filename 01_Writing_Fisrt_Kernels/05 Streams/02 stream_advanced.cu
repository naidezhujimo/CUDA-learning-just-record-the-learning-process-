...
int main(void) {
    ...
    // 分配主机端的 pinned 内存，这种内存可以更快地进行主机和设备之间的数据传输
    ...
    // 使用 cudaDeviceGetStreamPriorityRange 获取设备支持的流优先级范围
